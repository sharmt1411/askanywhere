...
class ApiLLM:
    ...
    def handle_user_query(select, context="none", question="none", callback=None):
        ...
        prompt_template = f"""
            ## 需求背景：
            你是一个智能ai聊天工具的辅助系统Jarvis，监测用户与AI的聊天过程，针对用户查询，分析是否需要从他的笔记库中调取资料。这个任务比较复杂，需要多个步骤来完成。
            
            ## 自定义指令：
            ### 1.触发器：用户输入内容
            ### 指令：深呼吸，仔细阅读用户的每一句话，理解用户的意图。
            
            ### 2.触发器：理解用户意图完成。
            ### 指令：分析是否需要从笔记库中调取资料。
            
            ### 3.触发器：需要从笔记库中调取资料。
            ### 指令：确认已经正确理解用户想要查询的资料。分析是否是具体的事务搜索，还是一段时间内全部笔记的搜索。 
                         
            ### 4.触发器：是具体的事务搜索。
            ### 指令：
                    4.1.从<标签库>中，选取你认为最符合用户查询的相关标签，判断使用AND或者OR方式查询。
                    4.2.分析出用户查询的笔记中，最有可能提到的词语，如果不明确，不提供关键词搜索参数更保险。
                    4.3.识别时间范围：识别用户查询的时间范围，比如“最近”这个词汇，应提供最近至少一周的信息等。
                    4.4.按照输出要求给出搜索参数。
            ...
            ### 参数解释：
                仔细理解下方的搜索参数，对于你的工作十分重要，这是你可以调用存储查询函数，包括6个参数：
