...
def get_notes_by_date(date):
    ...
    print("get_notes_by_date",date, "records:", records)
    # 这里只是模拟返回一些笔记内容
    return records
...
def review():
    ...
    for interval in REVIEW_INTERVALS:
        ...
        while review_date in records:              # 如果记录中有该日期的记录，否则说明超期
            ...
            if record['current_interval'] == interval:         # 如果当前间隔与记录中的间隔相同，则复习

                notes = get_notes_by_date(review_date)
                if notes:
                    string_notes = "\n".join(notes)
                    notes_list.append(f"##{review_date}\n##间隔{interval}天的复习记录\n"+string_notes+"\n")
                    found_record = True
                    print(f"Reviewing notes for {review_date}: {notes}")
